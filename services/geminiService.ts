...
const getClient = (apiKeyOverride?: string) => {
    ...
    if (!apiKey) {
        ...
        throw new Error("API Key is missing. Please provide it or configure process.env.API_KEY.");
    }
    return new GoogleGenAI({ apiKey });
};

export const generateStickerLabels = async (imageFile: File, apiKey?: string): Promise<string[]> => {
    // We let the caller handle try/catch so we can detect auth errors
